...
def _freeze_except_reid(model):

    for v in model.module.roi_heads.transformer.parameters():
        v.requires_grad = False
    return model
...
def check_if_freeze_model(model, cfg):
    ...
    elif cfg.MODEL.FREEZE_TYPE == 'reid':
        print('Reid')
        model = _freeze_except_reid(model)
    elif cfg.MODEL.FREEZE_TYPE == 'all':
        print('all')
        model = _freeze_all(model)
    else:
