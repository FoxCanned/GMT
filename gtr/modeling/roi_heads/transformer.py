...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.traj_emb = nn.Embedding(num_embeddings=401, embedding_dim=d_model)
    ...
    def forward(self, src, pos_embed=None, mask=None, \
        ...
        if self.training:
            ...
            traj_feature = self.encoder(
                src+emb_tensor, src_key_padding_mask=mask, pos=pos_embed) # N x B x F encoder
            ...
            if self.num_decoder_layers == 0:
                return tgt.unsqueeze(0).transpose(1, 2), \
                    traj_feature.permute(1, 0, 2).view(B, N, f)

            ########用当前目标特征做 query从全部目标特征里取特征，可能冗余？#######
            hs = self.decoder(tgt, traj_feature, memory_key_padding_mask=mask,
                              pos=pos_embed, query_pos=query_embed) # M x B x F

        else:

            mask_his = torch.ones(len(src), dtype=torch.bool)
            mask_his[query_inds] = False
            ...
            ###########自注意力，聚特征#########
            traj_feature = self.encoder(
                his_feature+emb_tensor, src_key_padding_mask=mask, pos=pos_embed)
