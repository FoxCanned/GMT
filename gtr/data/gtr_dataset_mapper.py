...
class GMTDatasetMapper(DatasetMapper):
    @configurable
    def __init__(
            self,
            is_train: bool,
    ...
    def __call__(self, video_dict):
        """
        video_dict: {'video_id': int, 'images': [{'image_id', 'annotations': []}]} 同一个video_id 一起读入
        """
        if video_dict['view_num'] != -1:
            if self.is_train:
                num_frames = min(int(len(video_dict['images']) / video_dict['view_num']), self.train_len)
            else:
                num_frames = int(len(video_dict['images']) / video_dict['view_num'])
            st = np.random.randint(int(len(video_dict['images']) / video_dict['view_num']) - num_frames + 1)
            gen_image_motion = self.gen_image_motion and self.is_train and \
                               int(len(video_dict['images']) / video_dict['view_num']) == 1
            ...
            if gen_image_motion:
            ...
            elif self.sample_range == 5. and self.is_train:
                frames = int(len(video_dict['images']) / video_dict['view_num'])
                ed = min(st + int(self.sample_range * num_frames), frames)
                ...
                ind.append(ind[0])
                inds = ind[0]
                for q in range(video_dict['view_num'] - 1):
                    inds = inds + [j + frames * (q + 1) for j in ind[q + 1]]
                images_dict = copy.deepcopy([video_dict['images'][x] for x in inds])
            elif self.sample_range >= 1. and self.is_train:

                frames = int(len(video_dict['images']) / video_dict['view_num'])
                #####################use this!!!!!!!!!!!!!!!!!!#############
                # frames = int(video_dict['images'][st]['ids_range'])
                # seq_len = int(video_dict['images'][st]['len_seq'])
                ed = min(st + int(self.sample_range * num_frames), frames)
                num_frames = min(num_frames, ed - st)
                ind = []
                for i in range(video_dict['view_num']):
                    ind.append(sorted(
                        np.random.choice(range(st, ed), size=num_frames, replace=False)))
                inds = ind[0]
                for q in range(video_dict['view_num'] - 1):
                    inds = inds + [j + frames * (q + 1) for j in ind[q + 1]]
                    # inds = inds + [j+seq_len*(q+1) for j in ind[q+1]]
                # inds = [inds[i+j*num_frames]  for i in range(num_frames) for j in range(video_dict['view_num'])]
                images_dict = copy.deepcopy([video_dict['images'][x] for x in inds])
            else:  # zth train 还需要改
                frames = num_frames
                # frames = 100
                ind = [i for i in range(frames)]
                inds = ind
                for q in range(video_dict['view_num'] - 1):
                    inds = inds + [j + num_frames * (q + 1) for j in ind]
                images_dict = copy.deepcopy([video_dict['images'][x] for x in inds])
    ...
    def __call__(self, video_dict):
        ...
        if self.is_train:
            ...
            num_frames = int(len(video_dict['images']) / video_dict['view_num'])
        st = 0  # np.random.randint(int(len(video_dict['images'])/video_dict['view_num']) - num_frames + 1)
        gen_image_motion = self.gen_image_motion and self.is_train and \
                           int(len(video_dict['images']) / video_dict['view_num']) == 1
        ...
        ret = []
        for i, dataset_dict in enumerate(images_dict):
            image_V = utils.read_image(dataset_dict["file_name"].replace('img1', 'img1V_cut'), format=self.image_format)
            image_T = utils.read_image(dataset_dict["file_name"].replace('img1', 'img1T'), format=self.image_format)
            utils.check_image_size(dataset_dict, image_V)
            utils.check_image_size(dataset_dict, image_T)
            transforms = None
            aug_input_V = T.StandardAugInput(image_V)
            aug_input_T = T.StandardAugInput(image_T)
            if gen_image_motion:
                transforms = transforms_list[i]
                image = transforms.apply_image(image)
            elif transforms is None:
                transforms = aug_input_T.apply_augmentations(self.augmentations)
                image_V = transforms.apply_image(image_V)
                image_T = aug_input_T.image
            else:
                image = transforms.apply_image(image)

            image_shape = image_T.shape[:2]  # h, w
            dataset_dict_T = copy.deepcopy(dataset_dict)
            dataset_dict_V = copy.deepcopy(dataset_dict)
            dataset_dict_T["image"] = torch.as_tensor(np.ascontiguousarray(image_T.transpose(2, 0, 1)))
            dataset_dict_V["image"] = torch.as_tensor(np.ascontiguousarray(image_V.transpose(2, 0, 1)))

            if not self.is_train:
                dataset_dict_T.pop("annotations", None)
                dataset_dict_T.pop("sem_seg_file_name", None)
                dataset_dict_V.pop("annotations", None)
                dataset_dict_V.pop("sem_seg_file_name", None)

            if "annotations" in dataset_dict_T:
                for anno in dataset_dict_T["annotations"]:
                    if not self.use_instance_mask:
                        anno.pop("segmentation", None)
                    if not self.use_keypoint:
                        anno.pop("keypoints", None)
                ...
                del all_annos
                dataset_dict_T["instances"] = utils.filter_empty_instances(instances)
                dataset_dict_V["instances"] = utils.filter_empty_instances(instances)
            ret.append(dataset_dict_V)
            ret.append(dataset_dict_T)
        return ret
